(function () {
  ...
  function dossierHTML(data) {
    ...
    const news = (data.related_news || []).map((item) => `<div class="dossier-item news-item"><a class="title-link" href="${esc(item.source_url)}" target="_blank" rel="noopener">${esc(item.title)}</a><div class="news-meta">${esc(item.source_name || "출처 확인 필요")} · ${esc(fmtDate(item.published_at || ""))}${item.related_count > 1 ? ` · 관련 보도 ${esc(item.related_count)}건` : ""}</div>${item.excerpt ? `<p>${esc(item.excerpt)}</p>` : ""}</div>`).join("");
