...
function card(x){
 ...
 const research=pitch?`<p class="recommendation-reason">${esc(pitch.reason||x.reason)}</p><p class="recommendation-change"><b>자료에서 확인한 변화</b> ${esc(x.changed_fact||x.one_line_signal||'전후 변화는 아직 확인하지 못했습니다.')}</p>`:'',researchDetails=(B?.renderDetails(x.research,x.clue_id)||'')+(!pitch?(B?.renderBriefHtml(x.research)||''):'');
 ...
 return `<article class="discovery-card ${pitch?'discovery-proposal':'discovery-reference'}"><div class="discovery-meta"><span>${pitch?'추천 · 검토 전':'참고자료'}</span><span>${esc(pitch?x.headline:x.detector_label)}</span><time>${esc(x.event_date?'예정 '+x.event_date:'자료 '+C.date(x.sort_date))}</time></div><h3>${esc(heading)}</h3>${research}<div class="discovery-actions"><button data-discovery-project="${esc(x.clue_id)}" ${pitch?'data-discovery-angle="0"':''}>취재에 담기</button></div><details data-detail="${esc(x.clue_id)}"><summary>근거와 추적 내역</summary><div class="discovery-detail"><div class="discovery-actions">${B?.requestFor(x)?`<button class="discovery-research-button" data-discovery-research="${esc(x.clue_id)}" ${researchPending.has(B.requestFor(x).key)?'disabled':''}>${researchPending.has(B.requestFor(x).key)?'원문 비교 중…':x.research?'원문 다시 비교':'원문 비교'}</button>`:''}${links((x.sources||[]).slice(0,2))}</div>${researchDetails}${x.extracted_facts?.length?'<h4>원문 자동 추출 · 검수 전</h4>'+list(x.extracted_facts):''}${x.confirmed_facts?.length?'<h4>기존 분석의 확인 내용 · 자료 기준일 확인</h4>'+list(x.confirmed_facts):''}${x.reported?.length?'<h4>보도된 내용</h4>'+list(x.reported):''}${x.original_title?'<p>'+esc(x.original_title)+'</p>':''}${evidence.length?'<h4>원문 위치</h4>'+list(evidence):''}${x.previous_state?'<h4>비교 기준</h4><p>'+esc(x.previous_state)+'</p>':''}${x.hypothesis?'<h4>아직 확인하지 않은 가설</h4><p>'+esc(x.hypothesis)+'</p><p>'+esc(x.falsification)+'</p>':''}${x.background_relationships?.length?'<h4>기존 투자·사업 관계 · 이번 거래 참여 여부 미확인</h4>'+list(x.background_relationships.map(r=>r.investor+' · '+r.as_of))+links(x.background_relationships.map(r=>({label:r.investor+' 관계 출처',url:r.url}))):''}${x.related_sources?.length?'<h4>같은 기업·기관의 다른 보도 · 동일 사건 여부 미확인</h4>'+links(x.related_sources.map(s=>({...s,label:s.title}))):''}<h4>모든 출처</h4>${links(x.sources)}</div></details></article>`;
}
function render({accept=false}={}){
 ...
 const recommended=x=>x.research?.status==='ready'&&x.article_brief?.angles?.length;
 let rows=data.filter(x=>(view==='background'?x.lane==='background':x.lane==='current'&&(view==='references'?!recommended(x):recommended(x)))&&(!query||[x.headline,x.article_pitch,x.one_line_signal,...(x.entities||[])].join(' ').toLowerCase().includes(query)));
 const total=rows.length;if(!expanded&&!query)rows=B?B.shortlist(rows,view==='current'?3:6):rows.slice(0,view==='current'?3:6);
 const busy=loading||researching||Object.values(status).some(s=>s.state==='loading');
 const failed=Object.values(status).some(s=>s.state==='failed');
 const empty=query?'검색 조건에 맞는 결과가 없습니다.':view==='current'?(busy?'자료를 대조하고 있습니다. 추천할 근거가 확인된 기사부터 표시합니다.':failed?'일부 자료를 불러오지 못했습니다. 현재 확보한 자료에는 추천 기사가 없습니다.':'현재 확보한 자료에는 추천할 기사가 없습니다. 새 근거가 확인되면 여기에 표시합니다.'):'현재 표시할 자료가 없습니다.';
 const html=rows.length?rows.map(card).join(''):`<div class="discovery-empty">${esc(empty)}</div>`;
 ...
 $('#discoveryCounts').textContent=`추천 기사 ${ready}건 · 최근 참고자료 ${counts.current-ready}건`;
 const blocked=data.some(x=>/model_/.test(x.research?.error||''));
 $('#discoveryResearchStatus').textContent=researching?'원문과 기존 보도를 대조 중입니다.':blocked?'분석 연결이 지연되고 있습니다. 확보한 자료는 ‘검토 중인 자료’에서 볼 수 있습니다.':'';
 $('#discoveryMore').hidden=!(!expanded&&!query&&total>rows.length);$('#discoveryMore').textContent=`나머지 ${total-rows.length}건 보기`;
...
root.addEventListener('click',e=>{
 const tab=e.target.closest('[data-discovery-view]');if(tab){view=tab.dataset.discoveryView;expanded=false;for(const b of document.querySelectorAll('[data-discovery-view]')){b.classList.toggle('on',b===tab);b.setAttribute('aria-pressed',String(b===tab));}render({accept:true});}
