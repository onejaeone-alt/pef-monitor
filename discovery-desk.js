...
function card(x){
 ...
 const pitched=Boolean(x.article_pitch),title=pitched?x.article_pitch:x.headline,firstCheck=pitched?x.story_requirements?.[0]:x.questions?.[0]||x.next_action;
 const origin=pitched?`<p class="discovery-origin"><b>출발 신호</b><span>${esc(x.headline)}</span></p>`:'';
 const storyDetails=pitched?`${x.story_requirements?.length?'<h4>기사에 필요한 확인</h4>'+list(x.story_requirements):''}${x.comparison_targets?.length?'<h4>비교할 사례·자료</h4>'+list(x.comparison_targets):''}`:'';
 return `<article class="discovery-card${pitched?' has-pitch':''}"><div class="discovery-meta"><span>${esc(x.detector_label)}</span>${x.story_mode?`<span class="story-mode">${esc(x.story_mode)}</span>`:''}<span>${esc(x.fact_status==='보도'?'보도 기반 · 당사자 확인 필요':x.evidence?'원문 추출 · 검수 전':x.fact_status||'단서')}</span><time>${esc(x.event_date?'예정 '+x.event_date:'자료 '+C.date(x.sort_date))}</time></div><h3>${esc(title)}</h3>${origin}<p class="discovery-fact">${esc(x.one_line_signal||x.changed_fact)}</p><p class="discovery-reason">${esc(x.why_today||x.reason)}</p><div class="discovery-question"><b>${pitched?'첫 확인':'확인할 질문'}</b><p>${esc(firstCheck||'원문과 당사자 확인이 필요합니다.')}</p><span>${esc((x.contacts||[]).slice(0,3).join(' · '))}</span></div><div class="discovery-actions">${links((x.sources||[]).slice(0,2))}<button data-discovery-project="${esc(x.clue_id)}">취재에 담기 →</button></div><details data-detail="${esc(x.clue_id)}"><summary>${pitched?'기사 제안 근거·취재 항목':'근거·추가 질문'}</summary><div class="discovery-detail">${storyDetails}${pitched?'<h4>왜 오늘</h4><p>'+esc(x.why_today||x.reason)+'</p>':''}${x.extracted_facts?.length?'<h4>원문 자동 추출 · 검수 전</h4>'+list(x.extracted_facts):''}${x.confirmed_facts?.length?'<h4>기존 분석의 확인 내용 · 자료 기준일 확인</h4>'+list(x.confirmed_facts):''}${x.reported?.length?'<h4>보도된 내용</h4>'+list(x.reported):''}${x.original_title?'<p>'+esc(x.original_title)+'</p>':''}${evidence.length?'<h4>원문 위치</h4>'+list(evidence):''}<h4>추가 확인</h4>${list(x.questions)}${x.previous_state?'<h4>비교 기준</h4><p>'+esc(x.previous_state)+'</p>':''}${x.hypothesis?'<h4>아직 확인하지 않은 가설</h4><p>'+esc(x.hypothesis)+'</p><p>'+esc(x.falsification)+'</p>':''}${x.background_relationships?.length?'<h4>기존 투자·사업 관계 · 이번 거래 참여 여부 미확인</h4>'+list(x.background_relationships.map(r=>r.investor+' · '+r.as_of))+links(x.background_relationships.map(r=>({label:r.investor+' 관계 출처',url:r.url}))):''}${x.related_sources?.length?'<h4>같은 기업·기관의 다른 보도 · 동일 사건 여부 미확인</h4>'+links(x.related_sources.map(s=>({...s,label:s.title}))):''}<h4>모든 출처</h4>${links(x.sources)}</div></details></article>`;
}
function render(){
 const open=new Set([...document.querySelectorAll('[data-detail][open]')].map(e=>e.dataset.detail));
 data=C.build({...state,reviews});
 const counts={current:data.filter(x=>x.lane==='current').length,background:data.filter(x=>x.lane==='background').length,stories:data.filter(x=>x.lane==='current'&&x.article_pitch).length};
 let rows=data.filter(x=>x.lane===view&&(!query||[x.headline,x.article_pitch,x.story_mode,x.why_today,x.one_line_signal,...(x.entities||[])].join(' ').toLowerCase().includes(query)));
 ...
 $('#discoveryCounts').textContent=`최근 자료 ${counts.current}건 · 기사 제안 ${counts.stories}건 · 기존 분석 ${counts.background}건`;
 ...
 $('#status').textContent=Object.values(status).some(s=>s.state==='loading')?'수집 결과 연결 중…':reading?'공시 근거 확인 중…':`기사 제안 ${counts.stories}건 · 최근 취재거리 ${counts.current}건`;
}
