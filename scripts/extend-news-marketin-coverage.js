...
const MARK='// MARKETIN_EVENT_FIRST_COVERAGE_V3';
...
function patch(source){
  ...
  // Keep policy-LP discovery narrow. A naked "국부펀드" query produced unrelated
  // overseas sovereign-fund stories, while KIC/한국판 국부펀드 is core MarketIN scope.
...
function patchApi(source){
  ...
  const next="      const inScope = shouldKeep(item, target, jak.names);\n      // Relevance may rank an in-scope story but cannot reopen a story rejected\n      // by the MarketIN reporting-scope gate. q= remains diagnostic only.\n      if (!inScope && !directProbe) continue;";
