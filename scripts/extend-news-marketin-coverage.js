...
const API_MARK='// MARKETIN_HARD_RADAR_GATE_V1';
...
function patchApi(source){
  ...
  let s=source;
  const imported="const { clusterIssues, eventLabel, queries, shouldKeep, theme } = require('../lib/news-monitor');";
  const imported2="const { clusterIssues, eventLabel, hardExcludeFromRadar, queries, shouldKeep, theme } = require('../lib/news-monitor');";
  s=replaceOnce(s,imported,imported2,'API monitor import');
  const duplicate=`      if (!cleanUrl || !key || seenUrl.has(cleanUrl) || seenTitle.has(key)) continue;\n      seenUrl.add(cleanUrl); seenTitle.add(key);`;
  const gated=`      if (!cleanUrl || !key || seenUrl.has(cleanUrl) || seenTitle.has(key)) continue;\n      // Hard exclusions run before NewsReader relevance rescue and before explicit query probes.\n      // A price-reaction headline must never become a reporting item merely because it mentions 자사주, M&A, etc.\n      if (hardExcludeFromRadar(item)) continue;\n      seenUrl.add(cleanUrl); seenTitle.add(key);`;
  s=replaceOnce(s,duplicate,gated,'API hard gate');
  return API_MARK+'\n'+s;
}

function main(root=path.resolve(__dirname,'..')){
  const monitor=path.join(root,'lib/news-monitor.js');
  const monitorBefore=fs.readFileSync(monitor,'utf8'),monitorAfter=patch(monitorBefore);
  if(monitorAfter!==monitorBefore)fs.writeFileSync(monitor,monitorAfter);
  const api=path.join(root,'api/news.js');
  const apiBefore=fs.readFileSync(api,'utf8'),apiAfter=patchApi(apiBefore);
  if(apiAfter!==apiBefore)fs.writeFileSync(api,apiAfter);
}
