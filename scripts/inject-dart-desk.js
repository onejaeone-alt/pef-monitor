...
function inject(html){
  ...
  let found=false;s=s.replace(js,()=>{if(found)return '';found=true;return '<script src="/dart-desk.js?v=6" defer></script>';});
  if(!found)s=s.replace('</body>','<script src="/dart-desk.js?v=6" defer></script></body>');
  const css=/<link\b[^>]*href=["']\/dart-desk\.css(?:\?[^"']*)?["'][^>]*>/g;
  found=false;s=s.replace(css,()=>{if(found)return '';found=true;return '<link rel="stylesheet" href="/dart-desk.css?v=6">';});
  if(!found)s=s.replace('</head>','<link rel="stylesheet" href="/dart-desk.css?v=6"></head>');
  return s;
}
