...
function addLeadHandoff(html) {
  ...
  let next = html;
  const re = /<button class="start-btn" data-package="package-\$\{i\}">(?:취재 질문 보기|취재 시작) ▾<\/button>/;
  const match = next.match(re);
  if (!match) return next;
  const questionButton = '<button class="start-btn" data-package="package-${i}">취재 질문 ▾</button>';
  next = next.replace(match[0], '<button class="project-send-btn" data-project-id="${esc(x.clue_id)}">내 취재로 보내기 →</button>' + questionButton);
  next = next.replace(/취재 시작 ▾/g,'취재 질문 ▾').replace(/취재 패키지 접기 ▴/g,'질문 접기 ▴').replace(/취재 질문 보기 ▾/g,'취재 질문 ▾').replace(/취재 질문 접기 ▴/g,'질문 접기 ▴');
...
for (const [file, fn] of [['leads.html',addLeadHandoff],['motae.html',addDossierSearchToMotae]]) {const target=path.join(root,file);if(!fs.existsSync(target))continue;const original=fs.readFileSync(target,'utf8');const next=fn(original);if(next!==original)fs.writeFileSync(target,next,'utf8');}
