...
function normalizeNav(html, file) {
  ...
  next = next.replace(/(<a[^>]*href="\/motae\.html"[^>]*>)모태펀드(<\/a>)/g, '$1출자공고$2');

  if (!next.includes('href="/leads.html"')) {
    const anchor = /(<a[^>]*href="\/motae\.html"[^>]*>출자공고<\/a>)/;
    next = next.replace(anchor, '$1<a href="/leads.html">AI 취재단서</a>');
  }
  if (!next.includes('href="/projects.html"')) {
    const anchor = /(<a[^>]*href="\/leads\.html"[^>]*>AI 취재단서<\/a>)/;
    next = next.replace(anchor, '$1<a href="/projects.html">내 취재</a>');
  }
...
function workflowMini(file) {
  ...
  return `<div class="workflow-mini" aria-label="취재 흐름">${link('lead','/leads.html','발견')}<span class="arrow">→</span>${link('project','/projects.html','취재')}<span class="arrow">→</span>${link('judge','/judgment.html','판단')}<span class="arrow">→</span><a href="https://article-engine-wjy-onejess.vercel.app/" target="_blank" rel="noopener">작성 ↗</a></div>`;
}
...
function simplifyLead(html, file) {
  ...
  next = next.replace('발견함입니다. 공시·출자·GP·펀드·뉴스를 이전 상태와 비교해 달라진 점과 확인할 질문만 찾습니다. 기사화 여부는 여기서 판단하지 않습니다.', '이전 상태와 비교해 달라진 점만 보여줍니다. 취재할 건은 ‘내 취재’로 보냅니다.');
  next = next.replace('공시·출자·GP·펀드·뉴스를 이전 상태와 비교해 기자가 확인할 변화를 먼저 찾습니다.', '이전 상태와 비교해 달라진 점만 보여줍니다. 취재할 건은 ‘내 취재’로 보냅니다.');
  next = next.replace('AI 취재단서는 발견함입니다. 단서를 고르고 사실을 확인한 뒤 기사판단기로 넘깁니다. 여기서는 기사화 결정을 내리지 않습니다.', '발견된 변화는 직접 확인 전까지 기사 사실로 쓰지 않습니다.');
