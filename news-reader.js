...
const focusBox=el('section',{id:'readerFocus',class:'reader-focus',hidden:true,'aria-labelledby':'readerFocusTitle'});$('.nd-list-heading').before(focusBox);
...
const exclusiveBox=btn('단독만 보기',{id:'readerExclusive',class:'nd-filter','data-exclusive':'toggle','aria-pressed':false});$('#actorFilters').after(exclusiveBox);
...
function accountDialog(){
 ...
 if(S.mode==='account'){
  content.push(el('p',{text:S.user.email+' 계정으로 로그인했습니다. 보관·관심 검색어은 다른 이용자와 공유하지 않습니다.'}),btn('다른 기기의 기록 불러오기',{id:'accountSync',class:'nd-button'}),btn('로그아웃',{id:'accountLogout',class:'nd-button'}));
...
function watchDialog(item){const entities=item?.related_entities||[];const input=el('input',{id:'readerWatchQuery',type:'text',maxlength:120,placeholder:'예: 운용사 이름 + 3호 펀드','aria-label':'추적 검색어'});if(entities[0])input.value=entities[0].canonical_name;
 openDialog('관심 검색어 등록',[el('p',{text:'여러 단어를 넣으면 모두 포함한 제목만 찾습니다. 특정 펀드는 운용사명과 펀드명을 함께 적어 주세요.'}),input,btn('관심 검색어 저장',{id:'readerWatchSave',class:'nd-button'}),...entities.map(e=>btn(e.canonical_name+' 소식 모아보기',{'data-watch-entity':e.entity_key,'data-watch-label':e.canonical_name,class:'nd-button'})),el('p',{class:'reader-fine',text:'현재 수집 범위에서 일치하는 보도를 찾습니다. 같은 사건인지, 새 사실인지는 본문 확인이 필요합니다.'})]);}
function render(){
 ...
 const collectionButton=([v,t])=>el('button',{type:'button','data-view':v,'aria-pressed':S.view===v,class:'nd-side-item'+(S.view===v?' is-active':'')},el('span',{text:t}),el('span',{class:'reader-category-count',text:count(v)}));
 const otherOpen=$('#readerOtherLists')?.open||['all','hidden','review'].includes(S.view);
 const other=el('details',{id:'readerOtherLists',class:'reader-other-lists'},el('summary',{text:'기타 목록'}),...[[ 'all','수집 기사 전체'],['hidden','숨긴 기사'],['review','선별 검토']].map(collectionButton));other.open=otherOpen;
 $('#collections').replaceChildren(...[['latest','최신 뉴스'],['tracked','관심 뉴스'],['saved','보관함']].map(collectionButton),other);
 ...
 exclusiveBox.textContent='단독만 보기 '+exclusiveCount;exclusiveBox.classList.toggle('is-active',S.exclusiveOnly);exclusiveBox.setAttribute('aria-pressed',String(S.exclusiveOnly));
 ...
 focusBox.replaceChildren(el('div',{class:'reader-focus-heading'},el('h3',{id:'readerFocusTitle',text:'먼저 볼 뉴스'}),el('span',{class:'reader-focus-count',text:'오늘 '+entries.length+'건 · 후속 확인 후보'})),criteria,
  entries.length?el('ol',{class:'reader-focus-list'},entries.map(entry=>{
   ...
   return el('li',{class:'reader-focus-row'+(S.selected===row.id?' is-selected':'')},el('button',{type:'button',class:'reader-focus-open','data-open':row.id},
    el('span',{class:'reader-focus-reason',text:entry.reasons.filter(r=>r.id!=='watch').slice(0,2).map(r=>r.label).join(' · ')}),
...
function rowNode(row){
 ...
 const badges=el('span',{class:'nd-row-top'},el('span',{class:'nd-category-pill','data-category':def.id,text:def.short}),watched?el('span',{class:'reader-watch-match',text:'관심 검색어 일치'}):null);
...
async function action(event){
 if(event.target.closest('[data-original]'))return;
 const b=event.target.closest('button');if(!b)return;const row=S.rows.find(x=>x.id===(b.dataset.save||b.dataset.hide||b.dataset.follow||b.dataset.open));
 try{
 ...
 else if(b.dataset.watchFilter){const w=S.records.watch[b.dataset.watchFilter];S.category='ALL';S.actor='ALL';S.exclusiveOnly=false;S.query=w.query||w.label;$('#search').value=S.query;setView('tracked');}
