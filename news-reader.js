...
const S={records:C.empty(),mode:'checking',user:null,authReady:false,view:'unread',newsScope:new URLSearchParams(location.search).get('scope')==='foreign'?'foreign':'domestic',category:'ALL',actor:'ALL',query:'',days:7,items:[],issues:[],rows:[],visible:[],selected:null,limit:30,loaded:false,pending:null,saving:false};
...
function translationDetail(item){
 ...
 return el('div',{class:'reader-translation'},el('p',{class:'reader-fine',text:item.title_ko?'한국어 자동번역':'한국어 번역을 불러오지 못했습니다. 새로고침하면 다시 시도합니다.'}),el('details',{},el('summary',{text:'영문 원제 보기'}),el('p',{lang:'en',text:item.title})),item.snippet_ko?el('p',{text:item.snippet_ko}):null);
}
...
function options(view=S.view){return {view,newsScope:S.newsScope,category:S.category,actor:S.actor,query:S.query,classify:classification};}
...
function switchScope(value){
 ...
 controller?.abort();sequence++;S.newsScope=value;S.items=[];S.issues=[];S.loaded=false;S.pending=null;S.selected=null;S.limit=30;S.query='';S.actor='ALL';S.category='ALL';$('#search').value='';$('#readerPending').hidden=true;$('#sourceNote').textContent='';$('#updatedAt').textContent='';
...
function rowNode(row){
 ...
 return el('article',{class:'nd-row reader-row'+(read?' is-read':'')+(S.selected===row.id?' is-selected':'')},el('button',{type:'button',class:'nd-row-main','data-open':row.id},badges,el('span',{class:'nd-row-title',text:headline(x)}),C.scope(x)==='foreign'?el('span',{class:'reader-translation-label',text:x.title_ko?'자동번역':'번역 재시도 필요'}):null,el('span',{class:'nd-row-meta',text:(x.source_name||'출처 확인 필요')+' · '+fmt(x.published_at)+(row.items.length>1?' · 관련 보도 '+row.items.length+'건':'' )})),el('div',{class:'reader-row-actions'},btn(read?'↶':'✓',{'data-read':row.id,title:read?'읽지 않음으로 되돌리기':'이 묶음 읽음 표시','aria-label':read?'읽지 않음으로 되돌리기':'이 묶음 읽음 표시',class:'reader-icon'}),btn(kept?'★':'☆',{'data-save':row.id,title:kept?'보관 해제':'보관','aria-label':kept?'보관 해제':'보관',class:'reader-icon'})));
}
