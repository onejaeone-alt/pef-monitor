...
function excerptEvidence(seed, source) {
  ...
  for (const b of blocks) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!topics.length || text.length < 10 || text.length > 320) continue;
      if (/로그인|저작권|개인정보처리|구독신청/.test(text)) continue;
      const words = text.split(/\s+/);
      if (source.kind !== 'official' && words.length > 24) continue;
      candidates.push({ text, topics, location: `${b.location || '본문'} · 추출문단 ${i + 1}` });
    }
  }
  // One short exact excerpt per document; a snippet is never an original quote.
  const chosen = candidates.find(x => x.topics.some(t => t.key === 'completion_claim')) || candidates.find(x => x.topics.some(t => t.key === 'deadline' || t.key === 'key_person')) || candidates[0];
  if (!chosen) return [];
