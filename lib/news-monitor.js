...
function queries(days) {
  ...
  return [
    ...
    `((연기금 OR 공제회 OR 보험사 OR 국민연금 OR 사학연금 OR 우정사업본부 OR LP) (출자 OR 자산배분 OR 위탁운용 OR "운용사 선정")) ${suffix}`,
    `((연기금 OR 공제회 OR 보험사 OR LP) (크레딧 OR 세컨더리 OR 공동투자) (출자 OR 투자 OR 약정)) ${suffix}`,
    `((연기금 OR 공제회 OR 보험사 OR LP) (재출자 OR 재약정 OR 분배금 OR "출자 축소" OR "약정액 축소")) ${suffix}`,
    ...
    `((IPO OR 기업공개 OR 상장예심 OR 수요예측 OR 공모가 OR 공모주 OR 상장주관) (상장 OR 거래소 OR 증권사 OR 기업)) ${suffix}`,
    `((PF OR 프로젝트파이낸싱 OR 크레딧 OR 신용등급 OR 신용위험 OR 회사채 OR 발행어음) (기업 OR 증권사 OR 건설 OR 금융 OR 부동산)) ${suffix}`,
    `((대체투자 OR 상업용부동산 OR 리츠 OR 인프라 OR 데이터센터) (기관투자 OR 연기금 OR 공제회 OR 운용사 OR PEF OR 사모펀드 OR 투자)) ${suffix}`,
    `((소액주주 OR 행동주의 OR 주주행동 OR 인적분할 OR 물적분할 OR 자사주 OR 지배구조) (상장사 OR 기업 OR 경영권 OR 주주)) ${suffix}`,
    `((금융위 OR 금융위원회 OR 금감원 OR 금융감독원 OR 거래소) (자본시장 OR 공시 OR 상장폐지 OR 모험자본 OR 초대형IB OR 생산적금융 OR 국민성장펀드 OR IPO)) ${suffix}`,
  ];
}

// An institution name alone is not an LP event. Match allocation/commitment
// activity before generic PE/VC words so these stories reach the LP lane.
const LP_ACTOR = /연기금|공제회|국민연금|사학연금|공무원연금|우정사업본부|보험사|생명보험|손해보험|삼성생명|삼성화재|한화생명|교보생명|한국성장금융|한국벤처투자|산업은행|기업은행|한국투자공사|\bKIC\b|\bLP\b|출자자/i;
const LP_ACTIVITY = /출자|자산\s*배분|위탁\s*운용|재약정|약정(?:액|금|규모)|분배금|(?:운용사|운용기관|\bGP\b).{0,12}(?:선정|교체|해임)|펀드.{0,12}(?:결성|조성)|(?:대체투자|크레딧|세컨더리|공동투자).{0,16}(?:비중|배정|투자|확대|축소|매입|매각)|(?:확대|축소|배정).{0,16}(?:대체투자|크레딧|세컨더리|공동투자)/i;
function isLpActivity(text) {
  const t = String(text || '');
  return LP_ACTOR.test(t) && LP_ACTIVITY.test(t);
}

function theme(text) {
  const t = String(text || '');
  if (isLpActivity(t)) return ['lp','LP·정책자금'];
  if (/사모펀드|PEF|프라이빗에쿼티|private equity|바이아웃|M&A|인수합병|공개매수|경영권|우선협상|본입찰|예비입찰|매각|인수/.test(t)) return ['ma_pef','M&A·PEF'];
  if (/벤처캐피탈|벤처 투자|CVC|기업형 벤처캐피탈|스타트업|시리즈[A-D]?|벤처투자/.test(t.replace(/한국벤처투자/g,''))) return ['vc','VC·스타트업'];
  if (/모태펀드|정책금융|출자사업|앵커LP/.test(t)) return ['lp','LP·정책자금'];
  if (/기업공개|\bIPO\b|상장예심|상장예비심사|수요예측|공모가|공모주|상장주관|스팩|SPAC/.test(t)) return ['ipo','IPO·ECM'];
  if (/회사채|인수금융|투자은행|IB업계|IB 업계|전환사채|교환사채|신주인수권부사채|메자닌|리파이낸싱|차환|유상증자|자금조달|매각주관사|프로젝트파이낸싱|\bPF\b|크레딧|신용등급|신용위험|발행어음/.test(t)) return ['ib','크레딧·기업재무'];
  if (/대체투자|상업용부동산|리츠|부동산투자회사|인프라투자|데이터센터/.test(t) && /기관|연기금|공제회|운용사|투자|사모|PEF|펀드|회수|대출|리파이낸싱/.test(t)) return ['alternative','대체투자·실물자산'];
  if (/소액주주|행동주의|주주행동|인적분할|물적분할|자사주|주주환원|지배구조/.test(t) && /기업|상장|주주|경영권|분할|소각|매입|철회/.test(t)) return ['governance','주주·지배구조'];
  if (/(금융위|금융위원회|금감원|금융감독원|한국거래소|거래소).{0,50}(자본시장|공시|상장폐지|모험자본|초대형\s*IB|생산적\s*금융|국민성장펀드|IPO)|(?:자본시장|공시|상장폐지|모험자본|초대형\s*IB|생산적\s*금융|국민성장펀드).{0,50}(금융위|금융위원회|금감원|금융감독원|한국거래소|거래소)/.test(t)) return ['policy','자본시장 정책·규제'];
...
function eventLabel(text) {
  const t = String(text || '');
  if (/회생|파산|워크아웃|채무불이행|부도|신용등급.{0,12}하향/.test(t)) return '회생·위험';
  if (isLpActivity(t)) return 'LP·출자';
  if (/우선협상|본입찰|예비입찰|SPA|주식매매계약/.test(t)) return 'M&A 진행';
  if (/매각|인수|M&A|인수합병|공개매수|경영권/.test(t)) return 'M&A';
  if (/기업공개|\bIPO\b|상장예심|상장예비심사|수요예측|공모가|공모주/.test(t)) return 'IPO';
  if (/프로젝트파이낸싱|\bPF\b|크레딧|신용등급|신용위험/.test(t)) return '크레딧·PF';
  if (/대체투자|상업용부동산|리츠|인프라투자|데이터센터/.test(t)) return '대체투자';
  if (/소액주주|행동주의|주주행동|인적분할|물적분할|자사주|주주환원/.test(t)) return '주주·지배구조';
  ...
  if (/상장|블록딜|세컨더리|회수|엑시트/.test(t)) return '회수·IPO';
...
function clusterIssues(items) {
  ...
  return clusters.map((cluster)=>{
    ...
    return {
      issue_id: crypto.createHash('sha1').update(key).digest('hex').slice(0,16), headline: latest.title,
      latest_seen: latest.published_at, first_seen: oldest.published_at, coverage_days: days.length,
      article_count: rows.length, source_count: sources.length, sources: sources.slice(0,8),
      theme_id: latest.theme_id, theme_label: latest.theme_label, event_label: latest.event_label,
      target: latest.target || null, related_entities: relatedEntities.slice(0,8),
      ongoing: rows.length >= 2 && sources.length >= 2, items: rows.slice(0,10),
    };
  }).sort((a,b)=>{
...
module.exports = { clusterIssues, eventLabel, normalizeTitle, queries, shouldKeep, theme };
