...
const CORE_SOURCES = [
  '이데일리','서울경제','한국경제','매일경제','머니투데이','조선비즈','더벨','인베스트조선','딜사이트','연합인포맥스',
  '연합뉴스','뉴스1','뉴시스','파이낸셜뉴스','아시아경제','헤럴드경제','이투데이','아주경제','블로터','뉴스핌','뉴스토마토',
  '동아일보','중앙일보','조선일보','한겨레','한국일보','경향신문','국민일보','세계일보'
];
...
function isCoreSource(source) {
  ...
  return CORE_SOURCES.some((name) => normalized.includes(normalizeSource(name)));
}
...
function strongIbContext(text) {
  return /사모펀드|PEF|프라이빗에쿼티|벤처캐피탈|CVC|인수금융|IB업계|IB 업계|우선협상|본입찰|예비입찰|공개매수|매각주관사|블라인드펀드|펀드레이징|모태펀드|한국벤처투자|한국성장금융|출자사업|재무적 투자자|전략적 투자자|전환사채|교환사채|메자닌|리파이낸싱/.test(String(text || ''));
}

function shouldKeep(item, target) {
  const text = `${item.title || ''} ${item.snippet || ''}`;
  if (hasNoise(text)) return false;
  const [themeId] = theme(text);
  if (themeId === 'other') return false;
  if (isCoreSource(item.source_name)) return true;
  return Boolean(target) || strongIbContext(text);
}
