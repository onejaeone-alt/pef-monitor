...
const OFFICIAL = ['kvic.or.kr','k-vic.co.kr','vcs.go.kr','nps.or.kr','kgrowth.or.kr','kdb.co.kr','mss.go.kr','korea.kr','fsc.go.kr','fss.or.kr','kcmi.re.kr','kvca.or.kr','assembly.go.kr','nanet.go.kr','kofia.or.kr','nars.go.kr','apfs.kr','cw.or.kr','kic.kr','ktcu.or.kr','poba.or.kr','geps.or.kr','tp.or.kr','mmaa.or.kr','sema.or.kr','pmaa.or.kr','koreapost.go.kr','kbiz.or.kr','mohw.go.kr'];
const MEDIA = ['edaily.co.kr','thebell.co.kr','dealsite.co.kr','hankyung.com','mk.co.kr','sedaily.com','newsis.com','yna.co.kr','yonhapnewstv.co.kr','platum.kr','venturesquare.net','mt.co.kr','fnnews.com','bloter.net','newstof.com','biz.chosun.com','news1.kr','newspim.com','news.einfomax.co.kr','bizwatch.co.kr','hani.co.kr','hankooki.com','inews24.com','newscj.com','munhwa.com'];
...
function htmlBlocks(html,kind='official') {
  ...
  const explicitlyFree=structured.some(x=>x.isAccessibleForFree===true||x.isAccessibleForFree==='true');
  // A free public article can include a separate login dialog for comments or bookmarks.
  // Access-denial pages and login text inside the article body remain blocked.
  if((blocked.test(bare)&&!explicitlyFree)||/Access Denied|Just a moment|자동입력 방지/i.test(bare)||/g-recaptcha|cf-chl-|captcha-form/i.test(html))throw new Error('ACCESS_LIMITED');
  const names=/^(articleBody|articleBodyContents|article-view-content-div|newsView|view_txt|newsView_txt|news_view_txt|news_body_area|news_body|newsBody|news_body_content|news_content|news_text|article_body|article-content|article_content|view-content|view_content|board_view_con|board-view-content|board-content|board_view_content|board-detail-content|view_cont|contentView)$/i;
  ...
  // A short UI placeholder may precede the actual body. Try explicit body containers,
  // never the complete media page, before declaring the article unreadable.
  if(core&&clean(decode(core.replace(/<[^>]*>/g,' '))).length<150){
    const bodyNames=/^(articleBody|articleBodyContents|article-view-content-div|view_txt|newsView_txt|news_view_txt|news_body_area|news_body|newsBody|news_body_content|news_content|news_text|article_body|article-content|article_content)$/i;
    const body=element(bare,(_,a)=>a.itemprop==='articleBody'||bodyNames.test(a.id||'')||(a.class||'').split(/\s+/).some(c=>bodyNames.test(c)));
    if(body&&clean(decode(body.replace(/<[^>]*>/g,' '))).length>=150)core=body;
    else if(ld){core=ld.articleBody;extractor='public_article_jsonld';}
  }
...
function publicationDate(html) {for(const m of html.matchAll(/<meta\b[^>]*>/gi)){const a=attrs(m[0]);if(/^(article:published_time|datePublished|pubdate)$/i.test(a.property||a.name||a.itemprop||'')&&/^20\d{2}-\d{2}-\d{2}/.test(a.content||''))return a.content;}for(const x of articleData(html))if(/^20\d{2}-\d{2}-\d{2}/.test(x.datePublished||''))return x.datePublished;
  const dateBlock=element(html,(_,a)=>(a.class||'').split(/\s+/).includes('veiw_date'));
  const published=clean(decode(dateBlock.replace(/<[^>]*>/g,' '))).match(/등록\s*(20\d{2}-\d{2}-\d{2})\s*(오전|오후)\s*(\d{1,2}):(\d{2}):(\d{2})/);
  if(published){const h=Number(published[3]);if(h>=1&&h<=12&&Number(published[4])<60&&Number(published[5])<60)return published[1]+'T'+String(h%12+(published[2]==='오후'?12:0)).padStart(2,'0')+':'+published[4]+':'+published[5]+'+09:00';}return null;}
