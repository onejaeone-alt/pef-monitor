...
function extractOrganization(anchorText, contextText, title) {
  ...
  const fromTitle = title.match(/^\[([^\]]{1,40})\]/)?.[1]?.trim();
...
function extractTitle(anchorText, contextText) {
  ...
  if (afterDeadline) return afterDeadline;
  ...
  const cleanAnchor = anchorText.replace(/^\s*상세보기\s*/,'').trim();
  if (cleanAnchor && !/^\d+$/.test(cleanAnchor)) return cleanAnchor;
  const contextMatch = contextText.match(/신청기한\s*:?\s*(?:19|20)\d{2}[-./]\d{2}[-./]\d{2}\s+(.+?)(?:\s+등록일자|\s+조회수|$)/)?.[1]?.trim();
  return contextMatch || cleanAnchor;
}
...
function extractNotice(anchorHtml, href, context) {
  ...
  title = title.replace(/^\[[^\]]{1,40}\]\s*/,'').trim();
