...
function managerCandidates(notice) {
  return [...new Set((notice?.manager_candidates || []).map(cleanText).filter(Boolean))];
}
  ...
  return (groups || []).map((group) => {
    ...
    const managerFormation = managers.map((manager) => {
      const matches = (funds || []).filter((fund) => {
        if (group.year && fund.year && String(group.year) !== String(fund.year)) return false;
        return fundMatchesManager(fund, manager);
      });
      return { manager, confirmed: matches.length > 0, funds: matches.slice(0, 5) };
    });
