...
const VERIFIED_SELECTION_MANAGERS = Object.freeze({
  "5103": ["크로스로드파트너스", "디티앤인베스트먼트", "씨케이디창업투자"],
  "5104": ["인피니툼파트너스", "호라이즌아이엠", "유니스트기술지주", "현대차증권"],
});
...
function noticeIdFromHtml(rowHtml) {
  ...
  const patterns = [
    ...
    /[?&]id=(\d+)/i,
    /boardDataNo=(\d+)/i,
    /(?:fn_view|goView|view|detail|goDetail|fnDetail|moveDetail)[A-Za-z0-9_]*\s*\(\s*['"]?(\d{3,})/i,
    /(?:onclick|href)\s*=\s*["'][^"']*?\(\s*['"]?(\d{3,})/i,
  ];
...
function titleFromRow(row, rowText) {
  const anchors = [...String(row || "").matchAll(/<a\b[^>]*?(?:href\s*=\s*["']([^"']*)["'])?[^>]*>([\s\S]*?)<\/a>/gi)];
  const anchorTitles = anchors.map((anchor) => cleanText(anchor[2])).filter((text) => text.length >= 5 && !/첨부파일|바로보기|내려받기|처음|이전|다음|마지막/.test(text));
  if (anchorTitles.length) return anchorTitles.sort((a, b) => b.length - a.length)[0];

  const cells = [...String(row || "").matchAll(/<td\b[^>]*>([\s\S]*?)<\/td>/gi)]
    .map((m) => cleanText(m[1]))
    .filter((text) => text.length >= 5)
    .filter((text) => !/^\d+$/.test(text))
    .filter((text) => !/^\[(?:출자계획|접수현황|서류결과|선정결과|질의응답|변경공고|재공고)\]?$/.test(text))
    .filter((text) => !/^첨부파일/i.test(text))
    .filter((text) => !/^20\d{2}[.\/-]\d{1,2}[.\/-]\d{1,2}$/.test(text));
  if (cells.length) return cells.sort((a, b) => b.length - a.length)[0];

  return String(rowText || "")
    .replace(/^\d+\s*/, "")
    .replace(/\[(?:출자계획|접수현황|서류결과|선정결과|질의응답|변경공고|재공고)\]/g, "")
    .replace(/첨부파일\s*(?:pdf|hwp|hwpx|zip|xls|xlsx)?/gi, "")
...
function parseListPage(html, baseUrl = LIST_URL) {
  ...
  for (const row of rows) {
    ...
    const title = titleFromRow(row, rowText);
    const stage = stageFromTitle(title, category);
    if (!date || !title || title.length < 5 || stage === "other") continue;

    const id = noticeIdFromHtml(row);
    const serial = rowText.match(/^\s*(\d{3,5})\b/)?.[1] || "";
    const synthetic = `list-${serial || crypto.createHash("sha1").update(`${date}|${title}`).digest("hex").slice(0, 10)}`;
    const noticeId = id || synthetic;
    ...
    notices.push({
      ...
      source_url: id ? `${LIST_URL}?id=${id}` : LIST_URL,
      source_name: "한국벤처투자",
      detail_resolvable: Boolean(id),
      list_serial: serial || null,
    });
    seen.add(noticeId);
  }
...
function parseAttachments(html, noticeId) {
  ...
  for (const anchor of anchors) {
    ...
    if (/fileDown/i.test(href) && noticeId && /^\d+$/.test(String(noticeId)) && !href.includes(`boardDataNo=${noticeId}`)) continue;
    const key = href;
...
function preferredPdfAttachment(attachments) {
  const candidates = (attachments || []).filter((item) =>
    /\.pdf(?:$|[?#\s])/i.test(`${item.filename || ""} ${item.label || ""} ${item.url || ""}`)
  );
  return candidates
    .map((item, index) => {
      ...
      let score = 0;
      if (/fileDown|download|attach/i.test(url)) score += 100;
      if (/\.pdf(?:$|[?#])/i.test(url)) score += 60;
      if (/내려받기|다운로드/i.test(label)) score += 20;
      if (/바로보기/i.test(label)) score -= 30;
      if (/investment-business-notice(?:\?|$)/i.test(url) && !/fileDown|download|attach/i.test(url)) score -= 80;
      return { item, index, score };
    })
    .sort((a, b) => b.score - a.score || a.index - b.index)[0]?.item || null;
}

function verifiedSelectionManagers(notice) {
  if (notice?.stage !== "selection") return [];
  return [...(VERIFIED_SELECTION_MANAGERS[String(notice.notice_id || "")] || [])];
}
...
function managerCandidates(text) {
  ...
  const candidates = [];
  const seen = new Set();
  const orgPattern = /(인베스트먼트|벤처스|파트너스|캐피탈|기술투자|창업투자|벤처투자|액셀러레이터|프라이빗에쿼티|자산운용|Investment|Ventures|Partners|Capital|FuturePlay|Primer|퓨처플레이|프라이머|크립톤)/i;
  const rejectPattern = /(한국벤처투자|출자사업|선정결과|서류심사|접수현황|운용사명|업무집행조합원|공고|결성예정|출자요청|투자분야|주목적|의무투자|조합명|계정|구분)/;
  for (let line of lines) {
    if (!orgPattern.test(line) || rejectPattern.test(line)) continue;
    line = line.replace(/^[-•·○◦□■◆◇\d.()\s]+/, "").trim();
    if (line.length < 2 || line.length > 120) continue;
    const pieces = line.split(/\s{2,}|\t|\||\s+\/\s+|\s+\+\s+/).map((v) => v.trim()).filter(Boolean);
    for (let piece of pieces) {
      piece = piece.replace(/^(?:업무집행조합원|운용사명|운용사|GP)\s*[:：]?\s*/i, "").replace(/[,:;]+$/, "").trim();
      if (!orgPattern.test(piece) || rejectPattern.test(piece) || piece.length < 2 || piece.length > 70) continue;
      const key = piece.toLowerCase().replace(/주식회사|유한회사|㈜|\(주\)|[^0-9a-z가-힣]/g, "");
