...
function isNewsAnnouncement(title){return !/[?？]|사설|칼럼|기자수첩|되겠다|반대/.test(title)&&/발표|공개|의결|확정|개정|시행|출범|도입|인가|허용|협약|체결|운용계획|투자계획|자산배분|수익률.{0,20}(?:기록|달성)/.test(title);}
...
async function collectNews(source,reader,deadline,H){
 ...
 const found=all.filter(x=>Date.parse(x.published_at)>Date.now()-30*86400000&&candidate(x.title,source,H)&&(H.isCandidate(x.title)||isNewsAnnouncement(x.title)));
...
module.exports={collectNews,QUERIES,isNewsAnnouncement};
