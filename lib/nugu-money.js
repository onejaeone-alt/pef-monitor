...
const NUGU_MONEY_REQUEST_HEADERS = {
  Accept: "application/json, text/plain, */*",
  Origin: NUGU_MONEY_SOURCE_URL.replace(/\/$/, ""),
  Referer: NUGU_MONEY_SOURCE_URL,
  "User-Agent": "PEF-Monitor/1.0 (+https://pef-monitor.vercel.app)",
};
...
async function fetchNuguItems({
  ...
  try {
    const response = await fetchImpl(NUGU_MONEY_ENDPOINT, {
      cache: "no-store",
      headers: NUGU_MONEY_REQUEST_HEADERS,
      redirect: "follow",
