...
const VERSION='marketin-research-3',CACHE_VERSION='daily-pitch-4';
...
angles: 기존 보도와 다른 후속 취재 방향 최대 3개. 같은 사실에서 질문과 증명 방법이 실제로 다를 때만 여러 개를 제시한다. 구체적인 가제 headline, 이 기사에서 새로 설명하려는 내용을 2문장으로 쓴 reason, 기존 마켓인 기사에서 더 나아갈 부분 new_information, 사건 고유의 핵심 취재 질문 question, 가장 먼저 메울 근거 빈칸 missing, 누구에게 어떤 자료나 답을 확인할지 적은 first_action, 이 가설이 틀렸다고 판단할 구체적 결과 falsification, 안정적인 direction_key, basis_ids와 coverage_ids를 적는다. coverage_ids는 비교 가능한 마켓인 기보도가 있을 때만 넣고 없으면 빈 배열이다. 누구에게나 붙는 ‘영향은’, ‘과제는’, ‘향방은’, ‘다음은’, ‘가격·자금원 점검’ 같은 표현은 쓰지 않는다. 제목에 미확인 내용을 확정하지 않는다.
