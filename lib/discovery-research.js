...
function chooseSources(records,topic,now){
 ...
 const own=all.filter(d=>marketin(d)||d.search_purpose==='marketin_coverage').slice(0,2),official=all.filter(d=>Sources.publicKind(d.url)==='official').slice(0,2),rest=all.filter(d=>!own.includes(d)&&!official.includes(d));
...
async function research({topic,seeds=[]},{search=Sources.search,read=Sources.readDocument,generate=synthesize,key=process.env.OPENAI_API_KEY,now=Date.now()}={}){
 ...
 const searches=await Promise.allSettled([search(`"${topic}" when:7d`,'latest',searchEnd),search(`"${topic}" site:marketin.edaily.co.kr when:45d`,'marketin_coverage',searchEnd),search(`"${topic}" (site:fsc.go.kr OR site:dart.fss.or.kr OR site:kgrowth.or.kr OR site:kvic.or.kr OR site:nps.or.kr) when:45d`,'official',searchEnd)]);
...
module.exports={VERSION,MODEL,PROMPT,schema,validate,chooseSources,research,cachedResearch,synthesize};
