...
reason은 기사 제목의 반복이나 보도량 설명이 아니라, 확인한 사실이 어느 거래 조건·자금 부담·주주 권리·결성 기한에 왜 중요한지 설명한다. question은 본문에서 이미 답한 질문을 반복하지 않는다. first_action은 웹과 공개문서에서 먼저 할 수 있는 확인으로 쓴다. 공개매수신고서와 결과보고서의 어떤 수량을 대조할지, 정정공시의 어떤 항목을 읽을지처럼 문서명과 확인할 항목을 명시한다. 전화나 오프라인 취재만으로 가능한 질문은 그 한계를 missing에 밝히고 공개자료에서 할 수 있는 작업을 먼저 제안한다. 원문으로 새로운 사실을 확인하지 못한 상태에서 기사 제목을 발견한 특징으로 되풀이하지 않는다.
...
function validate(output,docs){
 ...
 const angles=output.scope==='supported'?(output.angles||[]).filter(a=>addsEditorialQuestion(a,docs)&&a.basis_ids?.length>=2&&a.basis_ids.every(id=>seen.has(id))&&new Set(a.basis_ids.map(id=>facts.find(f=>f.id===id).source_id)).size>=2&&a.coverage_ids?.length&&a.coverage_ids.every(id=>coveredIds.has(id))&&[a.headline,a.reason,a.new_information,a.question,a.missing,a.first_action,a.falsification,a.direction_key].every(x=>clean(x)&&clean(x).length<=500)&&numbersSupported([a.headline,a.reason,a.new_information,a.question,a.missing,a.first_action,a.falsification].join(' '),a.basis_ids.map(id=>facts.find(f=>f.id===id).text).join(' '))).filter(a=>{const k=clean(a.question).replace(/[^a-z0-9가-힣]/gi,'').toLowerCase();const direction=clean(a.direction_key).replace(/[^a-z0-9가-힣]/gi,'').toLowerCase();if(!direction||angleKeys.has('q:'+k)||angleKeys.has('d:'+direction))return false;angleKeys.add('q:'+k);angleKeys.add('d:'+direction);return true;}).slice(0,3):[];
...
async function research({topic,seeds=[]},{search=Sources.search,index=readMarketinIndex,read=Sources.readDocument,generate=synthesize,key=process.env.OPENAI_API_KEY,now=Date.now()}={}){
 ...
 const searches=await Promise.allSettled([search(`"${topic}"${focus} when:7d`,'latest',searchEnd),search(`"${topic}"${focus} site:marketin.edaily.co.kr when:45d`,'marketin_coverage',searchEnd),search(`"${topic}"${focus} (site:fsc.go.kr OR site:dart.fss.or.kr OR site:kgrowth.or.kr OR site:kvic.or.kr OR site:nps.or.kr) when:45d`,'official',searchEnd),...(kind==='tender'?[search(`"${topic}" 공개매수 (신고서 OR 결과보고서 OR 최소 OR 선행조건) when:45d`,'previous_terms',searchEnd)]:[]),index(topic,searchEnd).then(records=>({records,log:[{provider:'publisher_public_indexes',status:'ok',count:records.length}]}))]);
