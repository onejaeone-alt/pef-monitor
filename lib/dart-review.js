...
function tables(markup) {
  ...
  return found.sort((a,b)=>a.start-b.start).map((t, ti) => ({ number:ti+1, start:t.start, rows:[...t.html.matchAll(/<TR\b[^>]*>([\s\S]*?)<\/TR>/gi)].map((m,ri)=>({ number:ri+1, cells:[...m[1].matchAll(/<(TD|TH|TE|TU)\b([^>]*)>([\s\S]*?)<\/\1>/gi)].map(c=>({value:text(c[3]),span:Math.max(1, Number(c[2].match(/COLSPAN\s*=\s*["']?(\d+)/i)?.[1]||1)),rowspan:Math.max(1,Number(c[2].match(/ROWSPAN\s*=\s*["']?(\d+)/i)?.[1]||1))})) })) }));
}
function header(row) {
...
function analyzeMarkup(markup,{rcept_no,entry='공시 원문'}={}) {
  ...
  const conflicts=new Set(),seenChanges=new Map();
  for(const c of changes){const k=key(c.label),prior=seenChanges.get(k);if(prior&&(!sameValue(prior.before,c.before)||!sameValue(prior.after,c.after)))conflicts.add(k);else seenChanges.set(k,c);}
