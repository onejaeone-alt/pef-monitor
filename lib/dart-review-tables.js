...
function parseTables(markup,text){
  ...
  return found.sort((a,b)=>a.start-b.start).map((t,ti)=>{
    ...
    return {number:ti+1,start:t.start,rows:[...html.matchAll(/<TR(?=[\s>])[^>]*>([\s\S]*?)<\/TR\s*>/gi)].map((m,ri)=>({number:ri+1,cells:[...m[1].matchAll(/<(TD|TH|TE|TU)(?=[\s>])([^>]*)>([\s\S]*?)<\/\1\s*>/gi)].map(c=>({value:text(c[3]),nested:c[3].includes('<NESTED-TABLE/>'),span:Math.max(1,Number(c[2].match(/COLSPAN\s*=\s*["']?(\d+)/i)?.[1]||1)),rowspan:Math.max(1,Number(c[2].match(/ROWSPAN\s*=\s*["']?(\d+)/i)?.[1]||1))}))}))};
  });
}
