...
async function handle(req,res){
 ...
 return res.status(200).json({ok:true,scope:'foreign',items:items.slice(0,limit),review_items:[],issues:[],source_policy:{name:'해외 경제·투자 전문매체',sources:SOURCES,language:'en'},providers:{google_news_rss:true},collection_status:{refresh:fresh,partial,succeeded,failed:qs.length-succeeded,truncated:items.length>limit},range:{days},stats:{scanned:raw.length,kept:Math.min(limit,items.length)},fetched_at:new Date().toISOString()});
