...
function shouldKeep(item) {
  ...
  if (id === 'periodic') return Number(item?.analysis?.entity_strength || 0) >= 2;
  return true;
}
...
function enrich(item) {
  const [group_id, group_label] = groupFor(item);
  const tier = tierFor(item);
  return {
    ...item,
    family_id: familyKey(item),
    base_report_nm: baseTitle(item.report_nm),
    group_id,
    group_label,
    tier,
    tier_label: tierLabel(tier),
    monitor_reason: monitorReason(item),
    next_check: nextCheck(item),
  };
}
...
module.exports = { buildFamilies, enrich, familyKey, groupFor, shouldKeep };
