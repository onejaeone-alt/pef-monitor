...
const { findWatchTarget } = require('./watch-config');
...
const MARKET_DISCOVERY_EVENTS = new Set([
  'control_change','equity_acquisition','equity_disposal','merger_restructuring','distress_legal'
]);

function refineEvent(item) {
  const title = String(item?.report_nm || '');
  const analysis = { ...(item?.analysis || {}) };
  if (/(전환가액|행사가액|교환가액).*(조정|변경)|전환청구권행사|신주인수권행사|자기주식.*(취득|처분)/.test(title)) {
    analysis.event_id = 'routine_security';
    analysis.event_label = '가격 조정·통상 주식 변동';
  } else if (/해산사유발생/.test(title)) {
    analysis.event_id = 'liquidation';
    analysis.event_label = '법인 해산';
  } else if (/(유형자산|자산양수도|부동산).*(양도|처분|매각)/.test(title)) {
    analysis.event_id = 'asset_disposal';
    analysis.event_label = '자산 매각';
  } else if (/최대주주등소유주식변동신고서/.test(title)) {
...
function groupFor(item) {
  ...
  if (['equity_acquisition','equity_disposal','asset_disposal','ownership_report'].includes(id)) return ['equity','지분·회수'];
  if (id === 'liquidation') return ['risk','법인 해산'];
...
function isCorrection(item) {
  return Boolean(item?.analysis?.is_correction) || /^\s*(?:\[(?:기재정정|첨부정정|정정)\]|기재정정|첨부정정|정정)/.test(String(item?.report_nm || ''));
}

function isSpac(item) {
  return /기업인수목적|스팩|SPAC/i.test(`${item?.corp_name || ''} ${item?.report_nm || ''}`);
}
...
function isSpecialWatch(match) {
  const target = findWatchTarget(match?.canonical_name || '');
  return target?.category === 'special';
}
...
function matchFitsWatchReason(item, match) {
  ...
  if (match.entity_type === 'company') {
    return isSpecialWatch(match) || hasPrivateFundSignal(title) || PORTFOLIO_CONTEXT.test(dossierContext(match));
  }
  return false;
}
...
function inReportingScope(item) {
  ...
  if (require('./dart-reporting-context').contextFor(item).relationships.length) return true;
  return reportingScopeMatches(item).length > 0;
}

function isMarketDiscovery(item) {
  const refined = refineEvent(item);
  const id = refined?.analysis?.event_id || 'general';
  const title = String(refined?.report_nm || '');
  const cls = String(refined?.corp_cls || '');

  if ((!MARKET_DISCOVERY_EVENTS.has(id) && id !== 'asset_disposal') || isCorrection(refined) || isSpac(refined)) return false;

  // 공개매수는 시장 구분과 관계없이 M&A 취재의 직접 신호라 남긴다.
  if (/공개매수/.test(title) && ['Y','K'].includes(cls)) return true;

  // Exact transaction titles can enter discovery, including unlisted acquisition vehicles.
  // Routine financing is still excluded; body evidence determines investor relevance.
  if (!['Y','K','E','N'].includes(cls)) return false;
  if (/타법인.*(취득|처분)결정|출자증권.*(취득|처분)결정|최대주주.*변경|경영권.*계약|주식양수도|합병결정|분할결정|영업양수|영업양도|주식교환|주식이전/.test(title)) return true;
  if (id === 'asset_disposal') return true;
  if (id === 'distress_legal' && /(회생절차|워크아웃|부도|대출원리금.*연체)/.test(title)) return true;
  return false;
}

function scopeKind(item) {
  if (inReportingScope(item)) return 'watch';
  if (isMarketDiscovery(item)) return 'market';
  return null;
}
...
function shouldKeep(item) {
  ...
  if (id === 'routine_security') return false;
  ...
  if (!directScope) return isMarketDiscovery(refined);
  if (id === 'periodic') return directScope;
...
function enrich(item) {
  ...
  const scope_kind = scopeKind(refined);
  const baseReason = monitorReason(refined);
  return {
    ...refined,
    ...
    scope_kind,
    scope_label: scope_kind === 'watch' ? '내 취재선' : scope_kind === 'market' ? '새 거래 발견' : '',
    monitor_reason: scope_kind === 'market' ? `새 거래 발견 · ${baseReason}` : baseReason,
    next_check: nextCheck(refined),
    reporting_context: require('./dart-reporting-context').contextFor(refined),
  };
}
...
module.exports = { buildFamilies, enrich, familyKey, groupFor, inReportingScope, isMarketDiscovery, matchFitsWatchReason, refineEvent, reportingScopeMatches, scopeKind, shouldKeep };
