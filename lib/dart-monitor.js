...
function matchFitsWatchReason(item, match) {
  ...
  if (match.entity_type === 'company') {
    // Curated special companies are intentionally monitored across their main
    // transaction/financing events. For other companies, the dossier itself
    // must show a PEF/M&A/exit/special-situation reason, or the filing must be
    // explicitly about a private-market fund. This keeps an incidental LP role
    // from opening every unrelated corporate filing.
