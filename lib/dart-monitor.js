...
function isMarketDiscovery(item) {
  ...
  if (!MARKET_DISCOVERY_EVENTS.has(id) || isCorrection(refined) || isSpac(refined)) return false;

  // 공개매수는 시장 구분과 관계없이 M&A 취재의 직접 신호라 남긴다.
  if (/공개매수/.test(title) && ['Y','K'].includes(cls)) return true;

  // 감시목록 밖의 일반 기업은 코스피의 신규 거래·구조조정만 발견 통로로 연다.
  // 자금조달·CB·담보·일상 지분변동은 여기서 열지 않는다.
  if (cls !== 'Y') return false;
  if (['control_change','equity_acquisition','equity_disposal','merger_restructuring'].includes(id)) return true;
