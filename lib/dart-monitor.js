...
const { matchDossiersInText } = require('./drive-dossiers');
...
function reportingScopeMatches(item) {
  ...
  if (!text) return [];
  return matchDossiersInText(text, 12).filter((match)=>PERSONAL_SCOPE_TYPES.has(match.entity_type));
}
...
function shouldKeep(item) {
  ...
  // The default DART desk is the reporter's monitored universe, not a generic
  // market-wide disclosure scanner. Ordinary listed-company events such as an
  // unrelated small-cap control-change correction stay out unless the company,
  // filer, GP/VC/LP house or portfolio company is already in the reporting OS.
  ...
  // Periodic reports are reference material only for directly monitored houses;
  // a portfolio company's routine quarterly report should not fill the desk.
  if (id === 'periodic') return refined.pef_entity === true;

  // Participation notices usually duplicate the actual capital-raise decision.
  // Keep only a filing made by a directly identified PEF/VC house itself.
...
module.exports = { buildFamilies, enrich, familyKey, groupFor, inReportingScope, refineEvent, reportingScopeMatches, shouldKeep };
