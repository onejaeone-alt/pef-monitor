...
function tierFor(item) {
  ...
  // SPAC expiry/dissolution can matter to an IPO reporter, but should not crowd
  // the first-look IB queue unless it is part of another transaction signal.
...
function shouldKeep(item) {
  ...
  const listed = ['Y','K','N'].includes(String(refined?.corp_cls || ''));
  if (id === 'general') return false;
  if (id === 'periodic') return strength >= 2;
  // Generic public funds are not PEF/VC fund-formation signals.
  if (id === 'fund_change') return hasPrivateFundSignal(title);
  // Participation notices duplicate the actual capital-raise decision in most
  // cases. Keep them only when the filer itself carries a strong monitored
  // private-market signal.
  if (/특수관계인의유상증자참여/.test(title)) return strength >= 1;
  // Unlisted affiliates can file large volumes of routine intra-group funding.
  // Keep listed-company cases and strong PEF/VC-linked entities; leave the rest
  // out of the reporting feed without deleting the source disclosure.
  if (!listed && strength < 1 && ['related_party_funding','related_party_equity','financing_support'].includes(id) && /특수관계인/.test(title)) return false;
  return true;
}
