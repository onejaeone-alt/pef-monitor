'use strict';
const Base=require('./preflight-core'),S=require('./preflight-sources'),Q=require('./preflight-question-evidence');
const C={...Base,buildCaseResult:(...args)=>Q.buildCaseResult(Base,...args),followupQueries:(...args)=>Q.followupQueries(Base,...args)};
...
function readPlan(refs,history,searches,seeds){
  // A per-case/per-search-purpose budget, not an article-value score.
...
async function runResearch(parent,options={}){
  ...
  steps.push({step:'followup',status:followed.some(g=>g.log.some(l=>l.status==='ok'))?'done':'partial',detail:'같은 대상의 완료 근거 후보가 있는지에 따라 후속 검색을 변경'});
  ...
  const searchFailed=!logs.some(l=>l.status==='ok'),partial=searchFailed||!historical.ok||!followed.length||cases.some(c=>!c.evidence.length)||sources.some(s=>s.access==='unread'||s.read_error&&!['READ_BUDGET_NOT_SELECTED','ORIGINAL_NOT_READ'].includes(s.read_error))||Date.now()>=deadline;
  ...
  if(funds.ready&&seeds.some(s=>s.playbook==='formation'&&!years.includes(s.year)))limitations.push(`조회된 공개 펀드목록의 연도는 ${years.join('·')||'미확인'}입니다. 조사 연도 자료가 없으면 현재 등록·결성 상태를 판단할 수 없습니다.`);
  return {ok:true,version:'preflight-1.2',run_id:C.hash(parent.clue_id,started),clue_id:parent.clue_id,title:parent.title,started_at:started,finished_at:new Date().toISOString(),canonical_as_of:seeds[0]?.canonical_as_of||null,state:partial?'partial':'completed',cases,
    sources:sources.map(({text,blocks,links,snippet,...meta})=>meta),coverage:{canonical_source:'기존 정본의 읽기전용 스냅샷',index_rows:historical.rows.length,index_ready:historical.ok,fund_catalog_ready:Boolean(funds.ready),fund_rows:(funds.items||[]).length,fund_catalog_years:years,searches:logs,body_read:bodyRead.length,relevant_body_read:relevantRead.length,official_attachments_read:bodyRead.filter(s=>s.found_by==='official_attachment').length,media_body_read:bodyRead.filter(s=>s.kind==='media').length,resolved_news_urls:sources.filter(s=>s.discovery_url).length,title_or_link_only:sources.filter(s=>!s.read_ok).length,evidence_count:evidenceCount,excluded_evidence_count:cases.reduce((n,c)=>n+c.excluded_evidence.length,0),complete_market_coverage:false},steps,limitations,
