...
function candidateMatches(r, seeds) {
  ...
  return seeds.some(s => title.includes(C.norm(s.gp || s.lp)) || (S.publicKind(r.url) === 'official' && title.includes(s.year) && s.tokens.some(t => title.includes(C.norm(t)))));
}
async function runResearch(parent, options = {}) {
  ...
  const attachments = dedupeSources(sources.filter(s => s.kind === 'official' && s.read_ok).flatMap(s => (s.links || []).map(l => ({ ...l, title:`${s.title} / ${l.title}`, found_by:'official_attachment' }))))
    ...
    .filter(s => /\.pdf(?:$|[?#\s])|\.hwpx?(?:$|[?#\s])/i.test(s.title + ' ' + s.url)).slice(0,2);
  ...
  const partial = allSearchFailed || !sources.some(s => s.read_ok) || sources.some(s => ['unread','attachment_unread'].includes(s.access)) || !historical.ok || Date.now() >= deadline || !followed.length;
