...
const DAY=86400000;
const SOURCES=[
 {id:'kgrowth-notice',name:'한국성장금융 출자사업 설명회',type:'growth',url:'https://www.kgrowth.or.kr/notice.asp?str_type=1&tab=1',organizer:'한국성장금융',pages:3},
 {id:'kgrowth-other',name:'한국성장금융 공시·기타 행사',type:'growth',url:'https://www.kgrowth.or.kr/notice.asp?str_type=2&tab=2',organizer:'한국성장금융',pages:3},
 {id:'kgrowth-press',name:'한국성장금융 발표·보도자료',type:'growth',url:'https://www.kgrowth.or.kr/notice.asp?str_type=3&tab=3',organizer:'한국성장금융',pages:3},
 {id:'kcmi',name:'자본시장연구원 행사',type:'kcmi',url:'https://www.kcmi.re.kr/',organizer:'자본시장연구원'},
 {id:'kvca',name:'벤처캐피탈협회·유관기관 행사',type:'kvca',url:'https://www.kvca.or.kr/Program/board/list.html?a_cd=5&a_gb=board&a_item=0&sm=3_1',limit:24},
 {id:'kvic-investment',name:'한국벤처투자 출자사업 설명회',type:'kvic',url:'https://www.kvic.or.kr/notice/kvic-notice/investment-business-notice',organizer:'한국벤처투자'},
 {id:'kvic-other',name:'한국벤처투자 기타 공지',type:'kvic',url:'https://www.kvic.or.kr/notice/kvic-notice/other-notice',organizer:'한국벤처투자'},
 {id:'kvic-press',name:'한국벤처투자 보도자료',type:'kvic',url:'https://www.kvic.or.kr/notice/kvic-news/press-release',organizer:'한국벤처투자'},
...
function pageUrls(html,source,H){
 ...
 for(const x of H.links(html,source.url)){
  ...
  if([...u.searchParams.keys()].some(k=>/^(page|pageIndex|pageNo|curPage)$/i.test(k))&&u.href!==base.href)out.push(u.href);
 }
...
async function collect(source,reader,deadline,previous,H){
 ...
 if(source.query){
  ...
  candidates=rss.filter(x=>source.id==='policy'?/주간|일정|보도계획/.test(x.title):H.isCandidate(x.title)).map(x=>({...x,url:x.source_url}));
 }else{
 ...
 await mapLimit(selected,4,async x=>{
  ...
  if(meta.title&&(!meta.published_at||Date.parse(meta.published_at)>=Date.now()-90*DAY))pending.push({id:H.key(x.url),title:meta.title,source_url:meta.url,source_id:source.id,source_name:source.name,published_at:meta.published_at||null,checked_at:new Date().toISOString(),reason});
 });
