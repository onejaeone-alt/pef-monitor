...
function fundYearNumber(value) {
  const match = String(value || "").match(/(?:19|20)\d{2}/);
  return match ? Number(match[0]) : 0;
}

function recentFundWindow(items, yearCount = 3) {
  const years = [...new Set((items || []).map((item) => fundYearNumber(item.year)).filter(Boolean))]
    .sort((a, b) => b - a)
    .slice(0, Math.max(1, yearCount));
  const allowed = new Set(years);
  const recent = (items || [])
    .filter((item) => !years.length || allowed.has(fundYearNumber(item.year)))
    .sort((a, b) => fundYearNumber(b.year) - fundYearNumber(a.year)
      || String(a.manager || "").localeCompare(String(b.manager || ""), "ko")
      || String(a.association_name || "").localeCompare(String(b.association_name || ""), "ko"));
  return { items: recent, years, latest_year: years[0] || null };
}
