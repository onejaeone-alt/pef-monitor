...
async function generate(rows,{key,fetcher=fetch,signal}={}){
 ...
 if(!r.ok){
  ...
  throw Error(r.status===401?'translation_key_invalid':code==='insufficient_quota'?'translation_quota_exhausted':code==='rate_limit_exceeded'?'translation_rate_limited':r.status===429?'translation_quota_or_rate_limit':'translation_provider_error');
 }
...
async function translateItems(items,{env=process.env,fetcher=fetch,storage,budgetMs=20000}={}){
 ...
 result.forEach((x,i)=>{const row=known.get(records[i].id);x.translation_id=records[i].id;x.translation_status=row?.status==='ready'?'translated':'unavailable';if(row?.status==='ready'){x.title_ko=row.title_ko;x.snippet_ko=row.snippet_ko;x.translation_provider=row.model==='assistant-session'?'ChatGPT':'OpenAI';}else if(!error&&row?.error_code){error=row.error_code;}});
...
async function lookup(id){if(!/^[a-f0-9]{64}$/.test(id))return null;const c=config();if(!c.url||!c.service)return null;const row=(await storeClient(c).read([id]))[0];return row?.status==='ready'?{id:row.id,title:row.source_title,title_ko:row.title_ko,snippet_ko:row.snippet_ko,provider:row.model==='assistant-session'?'ChatGPT':'OpenAI',model:row.model}:null;}
