...
async function readDocument(meta,deadline){
  const source={...metaSource(meta),parent_url:meta.parent_url||''};
  // Prefer exact-title search on a publisher's public index when supported.
  // This is independent of Google navigation; no consent or paywall bypass.
  if(source.kind==='search'){
    try{const url=await require('./publisher-url-lookup').resolveFromPublisher(source,boundedFetch,deadline);
      if(url){const result=await reader.read({...source,url},deadline);return {...result,discovery_url:source.url,attempts:['publisher_public_search',...(result.attempts||[])]};}
    }catch(_){/* A failed publisher search is not a missing article. The original link remains a candidate. */}
  }
  return reader.read(source,deadline);
}
