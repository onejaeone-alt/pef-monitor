...
function decode(v) { return String(v || '').replace(/&(#x[0-9a-f]+|#\d+|amp|quot|apos|lt|gt|nbsp|lsquo|rsquo|ldquo|rdquo|ndash|mdash);/gi, (m,k) => {
  if (k[0] === '#') { const n = k[1].toLowerCase() === 'x' ? parseInt(k.slice(2),16) : Number(k.slice(1)); return n > 0 && n <= 0x10ffff ? String.fromCodePoint(n) : ''; }
  return ({amp:'&',quot:'"',apos:"'",lt:'<',gt:'>',nbsp:' ',lsquo:'‘',rsquo:'’',ldquo:'“',rdquo:'”',ndash:'–',mdash:'—'})[k.toLowerCase()] || m;
}); }
...
function readLinks(html, base) {
  ...
  for (const m of String(html).matchAll(/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)) {
    if (!m[1] || /^#|^javascript:/i.test(m[1])) continue;
...
function metaSource(meta) {
  ...
  return { source_id: hash(url), url, title: clean(meta.title || meta.label || '공개자료'), published_at: meta.published_at || (meta.found_by === 'canonical_reference' ? null : meta.date) || null, reference_as_of: meta.found_by === 'canonical_reference' ? meta.date || null : null,
...
async function readDocument(meta, deadline) {
  ...
  try {
    ...
    if (meta.found_by === 'official_attachment' && !pdf) throw new Error('ATTACHMENT_NOT_READABLE_PDF');
    if (pdf) {
      ...
      if (domainIn(new URL(source.url).hostname, ['kvic.or.kr'])) {
        try { const detail = require('./kvic-notices').parseDetailPage(html, { source_url: source.url, notice_id: new URL(source.url).searchParams.get('id') });
          if (detail.posted_date) source.published_at = detail.posted_date;
    ...
    links = links.filter(l => { try { const u=new URL(l.url); return /fileDown|download|attach|\.(pdf|hwpx?)(?:$|[?#])/i.test(u.pathname); } catch (_) { return false; } });
...
function historyRecords(rows, seeds) {
  ...
  for (const r of rows || []) {
    ...
    const hit = seeds.some(s => {
      ...
      if (!norm(m.publisher).includes(norm(s.lp))) return false;
  ...
  return out.sort((a,b) => {
    const selected = v => /선정\s*결과/.test(v.title) ? 1 : 0;
    return selected(b)-selected(a) || distance(a)-distance(b);
  }).slice(0, 20);
}
