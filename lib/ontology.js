...
function watchTargetIdForName(value) {
  const normalized = normalizedName(value);
  return WATCH_TARGETS.find((target) => [target.name, ...(target.aliases || [])]
    .some((name) => normalizedName(name) === normalized))?.id || null;
}

function sameName(left, right) {
  const a = normalizedName(left);
  const b = normalizedName(right);
  const leftWatchId = watchTargetIdForName(left);
  const rightWatchId = watchTargetIdForName(right);
  const shortEnglishAlias = /^[a-z0-9]+$/.test(a) && /^[a-z0-9]+$/.test(b) && a.length >= 3 && b.length >= 3;
  return Boolean(a && b && (
    a === b
    || (leftWatchId && leftWatchId === rightWatchId)
    || ((shortEnglishAlias || (a.length >= 4 && b.length >= 4)) && (a.includes(b) || b.includes(a)))
  ));
}
...
function leadingOrganization(title) {
  ...
  if (/[?？…]|[‘“'\"]/.test(candidate) || /(매각|인수|투자|유치|선정|추진|결정|삼수|일까|인가)/.test(candidate)) return null;
...
function peopleIn(title) {
  ...
  for (const match of text.matchAll(pattern)) {
    ...
    if (/^(신임|전문|전문가|글로벌|투자|벤처|벤처투자|사외|공동)$/.test(name)) continue;
...
function uniqueSources(items) {
  ...
  return (items || []).filter((item) => {
    const key = item?.source_url || `${item?.title || ""}|${item?.published_at || ""}`;
...
function buildOntology(items) {
  ...
  function addDeal(item, owner, object) {
    ...
    const current = deals.get(key) || {
      ...
      currency: "KRW",
      summary: item.title,
      metadata: { source_signal_ids: [], participants: [], sources: [] },
    ...
    current.metadata.sources = uniqueSources([...(current.metadata.sources || []), {
      title: item.title,
      source_type: item.source_type || "other",
      source_name: item.source_name || null,
      source_url: item.source_url,
  ...
  for (const item of items || []) {
    ...
    const leadingMatchesAnotherWatchTarget = leading && watchTargetsIn(leading)
      .some((target) => !sameName(target.name, owner?.canonical_name));
    if (leading && !leadingMatchesAnotherWatchTarget && !sameName(leading, owner?.canonical_name)) {
