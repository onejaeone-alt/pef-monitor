...
function titleKey(value) {
  return String(value || '').replace(/<[^>]*>/g,' ').replace(/&#(x[0-9a-f]+|[0-9]+);/gi,(_,v)=>{const n=/^x/i.test(v)?parseInt(v.slice(1),16):Number(v);return n>0&&n<=0x10ffff?String.fromCodePoint(n):'';}).replace(/&apos;/gi,"'").replace(/&quot;|&ldquo;|&rdquo;/gi,'"').replace(/&amp;/gi,'&').replace(/&lsquo;|&rsquo;/gi,"'").replace(/\s*[-–|]\s*(?:플래텀(?:\(Platum\))?|Platum)\s*$/i,'').toLowerCase().replace(/[^a-z0-9가-힣]/g,'');
}
async function resolveFromPublisher(meta, transport, deadline) {
  ...
  const r = await transport(url,{deadline:Math.min(deadline,Date.now()+6000),maxBytes:150000});
  const posts = JSON.parse(r.buffer.toString('utf8').replace(/^\uFEFF/,''));
