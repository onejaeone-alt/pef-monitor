...
function classifyEvent(title, sourceType = "") {
  ...
  if (/(?:대표|파트너|심사역|본부장|임원|상무)[\s\S]{0,30}(?:영입|선임|취임|사임|퇴사|이동|독립|합류|승진)|(?:영입|선임|취임|사임|퇴사|이동|독립|합류|승진)[\s\S]{0,30}(?:대표|파트너|심사역|본부장|임원|상무)/.test(text)) return "people_move";
  if (/(?:시드|전략적|후속|신규|리드)\s*투자|투자\s*(?:유치|집행|참여|결정|단행|확대)|(?:유치|집행|참여)[\s\S]{0,15}투자|유상증자|전환사채|지분\s*(?:취득|투자)/.test(text)) return "investment";
...
function parseOfficialPage(html, source) {
  ...
  while ((match = anchorPattern.exec(String(html || "")))) {
    ...
    const actionable = /(20\d{2}년[\s\S]{0,80}(공고|모집|접수|선정|결과|출자)|\[(?:선정공고|접수현황|선정결과|공고|재공고)\]|(?:출자사업|위탁운용사|블라인드\s*펀드)[\s\S]{0,60}(?:공고|계획|모집|접수|선정|결과))/i.test(title);
    ...
    const bracketed = title.match(/^\[([^\]]{2,40})\]/)?.[1] || "";
    const subjectName = /선정|접수|공고|결과|안내|공지/.test(bracketed) ? "" : bracketed;
    results.push({
...
function interpretationFor(item) {
  const target = item.subject_name || item.target?.name || item.source_name || "해당 기관";
  ...
  if (item.event_type === "deal_process") return `${target} 관련 거래 절차가 움직였습니다. 매각 단계, 경쟁 원매자, 가격 기대치와 인수금융 확약 여부를 확인해야 성사 가능성을 판단할 수 있습니다. ${factText}.`;
...
const CLUSTER_STOPWORDS = new Set(["투자", "펀드", "결성", "조성", "유치", "관련", "위해", "통해", "국내", "신규", "재팬"]);

function clusterTokens(item) {
  const targetText = cleanText(`${item.target?.name || ""} ${(item.target?.aliases || []).join(" ")}`).toLowerCase();
  return new Set((cleanText(item.title).toLowerCase().match(/[0-9a-z가-힣]{2,}/g) || [])
    .filter((token) => token.length >= 3 && !CLUSTER_STOPWORDS.has(token) && !targetText.includes(token)));
}

function sameStory(left, right) {
  if (!left.target?.id || left.target.id !== right.target?.id) return false;
  const leftTokens = clusterTokens(left);
  const rightTokens = clusterTokens(right);
  const shared = [...leftTokens].filter((token) => rightTokens.has(token));
  if (shared.some((token) => token.length >= 5)) return true;
  const smaller = Math.min(leftTokens.size, rightTokens.size);
  return smaller > 0 && shared.length >= 2 && shared.length / smaller >= 0.45;
}

function collapseStoryClusters(items) {
  const kept = [];
  for (const item of items) {
    const match = kept.find((candidate) => sameStory(candidate, item));
    if (!match) {
      kept.push({ ...item, related_count: 1, related_sources: [{ title: item.title, source_name: item.source_name, source_url: item.source_url }] });
      continue;
    }
    match.related_count += 1;
    match.related_sources.push({ title: item.title, source_name: item.source_name, source_url: item.source_url });
  }
  return kept;
}

function withinDays(item, days, now = Date.now()) {
  const occurred = new Date(item.published_at || "").getTime();
  if (!Number.isFinite(occurred)) return false;
  const age = now - occurred;
  return age >= -6 * 3600 * 1000 && age <= days * 24 * 3600 * 1000;
}
...
async function collectReportingSignals({ days = 7 } = {}) {
  ...
  const ranked = dedupeSignals([...official, ...rawNews])
    .map(enrichSignal)
    .filter((item) => item.event_type !== "general")
    .filter((item) => withinDays(item, safeDays))
    .sort((a, b) => b.story_score - a.story_score || String(b.published_at || "").localeCompare(String(a.published_at || "")));
  const items = collapseStoryClusters(ranked);
  return {
    items,
    providers: {
