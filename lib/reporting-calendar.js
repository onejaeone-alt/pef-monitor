...
const DAY=86400000, TTL=30*60000, VERSION=5;
...
function isCandidate(title){return !EXCLUDE.test(title)&&EVENT.test(title);}
...
function normalizeDates(s){return String(s||'').replace(/[’‘'ʼ](\d{2})(?=\s*[년.\/\-])/g,'20$1').replace(/(\d{1,2})\.(?=\s*\([월화수목금토일]\))/g,'$1');}
...
function eventDate(text,reference){
 const lines=normalizeDates(text).split(/\n/);
 ...
 for(let i=0;i<lines.length;i++){
  ...
  if(l.length>1400||!EVENT.test(l)||!/개최(?:할|될|한다|하며|합니다|되는|된다|하고자|할 예정| 예정)|열(?:릴|리는|린다| 예정)/.test(l))continue;
...
async function getCalendar(){
 ...
 inflight=(async()=>{
  ...
  const sources=results.map((r,i)=>{
   ...
   if(r.status==='fulfilled'){fresh.push(...r.value.events);pending.push(...r.value.pending,...(old?.pending||[]).filter(x=>x.source_id===s.id&&!r.value.stats.processed_urls?.includes(x.source_url)&&Date.parse(x.checked_at||0)>now-7*DAY));}
...
module.exports={handle,getCalendar,pageText,endDate,kstDay,validDate,dateIn,eventDate,makeEvent,parseFsc,parseWeekly,mergeEvents,TOPIC,EXCLUDE,EVENT,isCandidate,SOURCES};
