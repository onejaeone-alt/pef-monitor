...
const DAY=86400000, TTL=30*60000, VERSION=10;
...
const EVENT=/투자로드쇼|컨퍼런스|콘퍼런스|세미나|포럼|토론회|공청회|간담회|설명회|발표회|서밋|summit|conference|교류회|네트워킹|투자\s*위크|핀테크\s*위크|엑스포|박람회|밋업|meetup|데모\s*데이|demo\s*day|딜\s*데이|IR(?:\b|행사|데이)|MUST\s*Round|Super\s*Return|워크숍/i;
...
function eventIdentity(e){
 ...
 return (named||e.title).replace(/^(?:자본시장감독국)\s*/,'').replace(/\s+\d{2}-\d{2}-\d{2}$/,'').replace(/\s|[「」『』“”]/g,'');
}
function mergeEvents(previous,fresh,now=Date.now()){const map=new Map(previous.map(e=>[e.id,e]));for(const e of fresh){const old=map.get(e.id);const changed=old&&['date','time','venue','status'].some(k=>old[k]!==e[k]);map.set(e.id,{...e,checked_at:new Date(now).toISOString(),...(changed?{changed_at:new Date(now).toISOString(),previous_date:old.date}:old?.changed_at?{changed_at:old.changed_at,previous_date:old.previous_date}:{})});}
 const seen=new Set();return [...map.values()].filter(e=>(e.end_date||e.date)>=kstDay(now-7*DAY)&&e.date<=kstDay(now+180*DAY)).sort((a,b)=>(a.date+a.time).localeCompare(b.date+b.time)).filter(e=>{const k=e.date+eventIdentity(e).replace(/㈜|\(주\)/g,'');if(seen.has(k))return false;seen.add(k);return true;}).slice(0,300);}
...
async function collect(source,reader,deadline,previous){return require('./reporting-calendar-sources').collect(source,reader,deadline,previous,{pageText,links,clean,key,kstDay,dateIn,makeEvent,parseFsc,parseWeekly,isCandidate,TOPIC,timeIn});}
...
async function getCalendar(){
 ...
 inflight=(async()=>{
  ...
  const events=mergeEvents((old?.events||[]).filter(e=>e.source_type!=='news'||isCandidate(e.title)||require('./reporting-calendar-lp-news').isNewsAnnouncement(e.title)),fresh,now),dated=new Set(events.map(e=>e.source_url));
...
async function handle(req,res){try{if(req.query?.inspect){res.setHeader('Cache-Control','no-store');return res.status(200).json({ok:true,sources:await require('./reporting-calendar-institutions').inspect(String(req.query.inspect).split(','))});}const payload=await getCalendar();res.setHeader('Cache-Control','public, max-age=60, s-maxage=120');return res.status(200).json({ok:true,...payload,sources:payload.sources.map(({checks,processed_urls,...s})=>s),today:kstDay()});}catch(_){return res.status(503).json({ok:false,error:'일정을 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.'});}}
module.exports={handle,getCalendar,pageText,endDate,kstDay,validDate,dateIn,eventDate,makeEvent,parseFsc,parseWeekly,mergeEvents,TOPIC,EXCLUDE,EVENT,isCandidate,eventIdentity,timeIn,clean,key,links,SOURCES};
