...
function catalog() {
  ...
  const rows = makeCatalog(buildCanonicalClues(), SNAPSHOT_DATE);
  // Search uses the common public name; canonical IDs, titles and source references stay unchanged.
  for (const entry of rows) for (const seed of entry.cases) {
    if (seed.lp === '국민연금공단 기금운용본부') { seed.lp_canonical = seed.lp; seed.lp = '국민연금'; }
  }
  return rows;
}
function rejection(req) {
  if (req.method && req.method !== 'GET') return 'METHOD';
  if (req.body && Object.keys(req.body).length) return 'BODY';
  // Vercel access links may add these transport-only parameters. Never pass them to search.
  const allowed = new Set(['mode','id','_','_vercel_share','_vercel_no_cache']);
  if (Object.keys(req.query || {}).some(k => !allowed.has(k))) return 'QUERY_FIELDS';
  const origin = req.headers?.origin;
  if (!origin) return null;
  try { const u = new URL(origin); return u.protocol === 'https:' && ['ainobi.news','www.ainobi.news','pef-monitor.vercel.app','pef-monitor-onejess.vercel.app'].includes(u.hostname) ? null : 'ORIGIN'; } catch (_) { return 'ORIGIN'; }
}
function allowedRequest(req) { return rejection(req) === null; }
async function handlePreflight(req,res) {
  res.setHeader('Cache-Control','no-store');
  res.setHeader('X-Content-Type-Options','nosniff');
  const invalid = rejection(req);
  if (invalid) return res.status(400).json({ok:false,error:'공개 단서 ID만 전달할 수 있습니다. 취재 메모·임의 URL·본문은 받지 않습니다.',request_check:invalid});
