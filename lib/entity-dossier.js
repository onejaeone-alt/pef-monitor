...
function buildProfileOverview(entity, profile) {
  ...
  const representatives = uniqueBy((profile?.connections || [])
    .map((item) => firstMatch(item, /^(?:대표[^:]*|CEO|설립자)\s*:\s*(.+)$/i))
    .filter(Boolean)
    .map(cleanRepresentative), (item) => item);
  const foundedYear = firstMatch(summary, /(19\d{2}|20\d{2})년\s*설립/);
  const assetsUnderManagement = firstMatch(summary, /((?:약\s*)?[0-9조억만,\s]+원)(?:을|를)\s*운용/);
  const portfolioCount = firstMatch(summary, /(\d+개\s*기업)(?:에|에\s*)\s*투자/);
