...
function baseQuestions(type, name) {
  if (["pef", "vc", "ac"].includes(type)) return [
    `${name}이 현재 모집 중인 펀드의 목표액과 실제 약정액은 얼마인가`,
  ...
  if (type === "person") return [
    `${name}이 현재 맡은 직책과 담당 펀드·투자 분야는 무엇인가`,
...
function relationQuestions(relations, name) {
  ...
  const questions = [];
  if (types.has("preferred_bidder_for")) questions.push(`${name}이 우선협상대상자로 선정된 거래의 가격과 배타적 협상기간은 얼마인가`);
...
function buildEntityDossier(graph, entityKey) {
  ...
  const evidence = uniqueBy([...relationEvidence, ...dealEvidence], (item) => item.source_url || `${item.title}|${item.published_at}`)
  ...
  return {
    entity,
    type_label: TYPE_LABELS[entity.entity_type] || "기관·기업",
    summary: `${entity.canonical_name}과 직접 연결된 관계 ${relations.length}건, 거래 ${deals.length}건, 근거 자료 ${evidence.length}건을 찾았습니다.`,
    relations,
    deals,
    evidence,
    questions,
    stats: { relations: relations.length, deals: deals.length, evidence: evidence.length },
  };
}
