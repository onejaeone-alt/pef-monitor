...
function cleanRepresentative(value) {
  return String(value || "")
    .replace(/^(?:PER|ORG|VAC)-[A-Z0-9-]+\s+/i, "")
    .trim();
}

function representativesFromText(value) {
  const names = [];
  const clauses = String(value || "").split(/[.!?]|,\s*/);
  const patterns = [
    /([가-힣]{2,4})\s+(?:대표이사|공동대표|단독대표)(?=\s|·|$)/g,
    /([가-힣]{2,4})(?:은|는|이|가)\s+(?:단독)?대표(?:이사)?(?:로|를|\s+체제)/g,
  ];
  for (const clause of clauses) {
    if (/사임|해임|퇴임|전\s*대표|이탈|떠났|물러/.test(clause)) continue;
    for (const pattern of patterns) {
      for (const match of clause.matchAll(pattern)) names.push(match[1]);
    }
  }
  return names;
}

function buildProfileOverview(entity, profile) {
  const summary = String(profile?.summary || "");
  const type = entity?.entity_type || "organization";
  const category = profile?.type_label || TYPE_LABELS[type] || "기관·기업";
  const basic = (profile?.current_status || []).find((item) => ["기본정보", "마스터"].includes(item.label))?.text || "";
  const profileText = [summary, ...(profile?.current_status || []).map((item) => item.text)].join(" ");
  const stageAndSectors = basic.match(/^(.+?)\s*중심\s*·\s*(.+)$/);
  const representatives = uniqueBy([
    ...(profile?.connections || [])
      .map((item) => firstMatch(item, /^(?:대표[^:]*|CEO|설립자)\s*:\s*(.+)$/i))
      .filter(Boolean)
      .map(cleanRepresentative),
    ...representativesFromText(profileText),
  ], (item) => item);
  const foundedYear = firstMatch(profileText, /(19\d{2}|20\d{2})년\s*설립/);
  const assetsUnderManagement = firstMatch(profileText, /((?:약\s*)?[0-9조억만,\s]+원)(?:을|를)\s*운용/)
    || firstMatch(profileText, /((?:약\s*)?[0-9조억만,\s]+원(?:대|\s*안팎|\s*이상)?)\s*(?:규모의?\s*)?(?:AUM|운용자산|운용규모)/i);
  const portfolioCount = firstMatch(profileText, /(\d+개\s*기업)(?:에\s*)?투자/);
  const categoryParts = category.split("·").map((item) => item.trim()).filter(Boolean);
  const serviceNames = {
    ac: "초기투자·액셀러레이팅",
    company: "기업 정보",
    financial_institution: "금융·투자",
    fund: "펀드",
    lp: "출자",
    pef: "사모펀드 운용",
    person: "인물 정보",
  ...
  return {
    category: categoryNames[type] || category,
    aliases: entity?.aliases || [],
    founded_year: foundedYear ? `${foundedYear}년` : null,
    representatives,
    investment_stage: stageAndSectors?.[1] || null,
    focus_sectors: stageAndSectors?.[2] || null,
    specialization: !stageAndSectors && categoryParts.length > 1 ? categoryParts.slice(1).join("·") : null,
    assets_under_management: assetsUnderManagement,
    portfolio_count: portfolioCount,
    service_name: serviceNames[type] || "기관 정보",
    service_description: summary || null,
    basis_date: profile?.updated_at || entity?.metadata?.last_seen_at || null,
  };
}
...
function buildEntityDossier(graph, entityKey) {
  ...
  const relations = (source.edges || [])
    ...
    .map((edge) => {
      ...
      return {
        ...
        ...relationInsight(edge.relation_type),
      };
    })
  ...
  return {
    ...
    summary: profile?.summary || `${entity.canonical_name} 관련 인물·운용사·펀드 ${relations.length}건, 거래 ${deals.length}건, 근거 자료 ${evidence.length}건을 모았습니다.`,
    ...
    evidence,
    questions,
    profile_overview: buildProfileOverview(entity, profile),
    stats: {
