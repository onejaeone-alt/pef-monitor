...
async function loadOntologyGraph(limit = 300) {
  ...
  const [entityResponse, relationResponse, dealResponse] = await Promise.all([
    ...
    request(`deals?select=id,deal_key,deal_name,deal_type,current_stage,status,target_entity_id,summary,metadata&order=updated_at.desc&limit=${safeLimit}`, { method: "GET", headers: headers() }),
  ]);
