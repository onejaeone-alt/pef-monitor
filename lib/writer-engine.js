...
function meaningFromEvidence(subject, tip, eventType, verification, evidence) {
  ...
  if (eventType === "distress" && /(?:회생계획안?|회생안)[\s\S]{0,15}인가/.test(titleText)) {
    const approvalTitles = titles.filter((title) => /(?:회생계획안?|회생안)[\s\S]{0,15}인가/.test(title));
    const vote = titleText.match(/채권자\s*([\d.]+)%\s*찬성/)?.[1];
    return {
      summary: `입력문과 반대에 가까운 흐름입니다. 최신 보도 ${approvalTitles.length || "여러"}건은 ${subject}의 회생계획안이 법원 인가를 받아 파산 고비를 넘기고 정상화 단계로 갔다고 전합니다.${vote ? ` 제목에 나온 채권자 찬성률은 ${vote}%입니다.` : ""} 기사로 쓰려면 법원 인가 결정문에서 채권 변제율·감자·출자전환·대주주 책임과 영업 정상화 조건을 확인해야 합니다.`,
      basis_titles: titles,
    };
  }
