...
function install(root){
 ...
 B.requestFor=function(x){const r=originalRequest.call(B,x);return r?{...r,key:'daily-pitch-3:'+r.key}:r;};
 ...
 B.renderBriefHtml=function(result,...args){
  ...
  if(base.includes('<section class="marketin-story-brief" aria-label="원문 기반 이슈 브리핑">'))return base.replace('<section class="marketin-story-brief" aria-label="원문 기반 이슈 브리핑">',`<section class="marketin-story-brief has-daily-pitch" aria-label="원문 기반 이슈 브리핑">${daily}`);
  return daily+base;
 };
 B.shortlist=function(rows,limit=6){return rows.filter(isDaily).sort((a,b)=>String(b.sort_date||'').localeCompare(String(a.sort_date||''))).slice(0,limit);};
 B.isDailyCandidate=isDaily;B.renderDailyPitch=render;B.__dailyReportPitchInstalled=true;
 if(root.document){
  const update=()=>{
   if(!root.document.querySelector('[data-discovery-view="current"].on'))return;
   const note=root.document.querySelector('#discoveryCounts');if(!note)return;const m=note.textContent.match(/최근 자료\s*(\d+)건\s*·\s*기존 분석\s*(\d+)건/);if(!m)return;
   const total=Number(m[1]),cards=root.document.querySelectorAll('.daily-report-pitch').length;note.textContent=`일보 후보 ${cards}건 · 탐색 중 ${Math.max(0,total-cards)}건 · 기존 분석 ${m[2]}건`;
   const more=root.document.querySelector('#discoveryMore');if(more&&!more.hidden&&more.textContent.includes('나머지'))more.textContent=`탐색 중 ${Math.max(0,total-cards)}건도 보기`;
  };
