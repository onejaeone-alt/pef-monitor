...
test('bounded MarketIN index excerpt and specific public headline remain explicitly limited evidence',()=>{
 ...
 const fluff=R.fallbackEvidence({url:'https://news.google.com/rss/articles/abcdefghijklmnop',title:'홈플러스 소식',publisher:'매체'},{read_ok:false});assert.notEqual(fluff?.read_ok,true);
});

test('daily report block states the usable pitch and one decisive gap',()=>{
...
test('default shortlist contains only daily candidates while underlying exploration rows remain intact',()=>{
 ...
 const root={MarketInStoryBrief:B};assert.equal(D.install(root),true);const result={status:'ready',analysis:{daily_pitch:{headline:'일보'}}};
 const daily=B.attach({clue_id:'d',sort_date:'2026-09-17'},result),explore=B.attach({clue_id:'e',sort_date:'2026-09-17'},{status:'ready',analysis:{daily_pitch:null}});assert.deepEqual(B.shortlist([explore,daily]).map(x=>x.clue_id),['d']);assert.equal([explore,daily].length,2);assert.match(B.renderBriefHtml(result,'issue-x'),/issue-x/);
});

test('AI discovery loads daily gate after reporting-direction wrappers and before the desk',()=>{
