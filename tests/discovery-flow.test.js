...
test('article count alone never creates a pitch when there is no next-stage or scenario cue',()=>{
 const rows=Array.from({length:6},(_,i)=>({title:'A사 신제품 출시 관련 보도 '+i,published_at:i<3?'2026-09-16':'2026-09-17',source_name:i%2?'매체B':'매체A',source_type:'domestic_news',source_url:'https://example.com/noise-'+i,target:{name:'A회사'},related_entities:[]}));
...
test('multi-day M&A reporting needs an explicit next-stage cue before it becomes a deal followup pitch',()=>{
 ...
 ].map(([title,published_at,source_name],i)=>({title,published_at,source_name,source_type:'domestic_news',source_url:'https://example.com/deal-'+i,target:{name:'B회사'},related_entities:[]}));
...
test('story-pitch project handoff uses the pitch as project title while preserving reporter edits',()=>{
