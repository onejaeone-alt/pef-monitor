...
function harness({error=false,empty=false}={}){
 ...
 const result=()=>({ok:true,version:B.VERSION,status:error?'sources_only':'ready',error:error?'model_unavailable':null,as_of:new Date().toISOString(),sources:[{...source,source_id:'s1',publisher:'마켓인',read_ok:true}],analysis:error?null:{summary:{text:'확인한 사실'},why_now:{text:'새 결성 자료 공개'},facts:[{id:'f1',source_id:'s1',text:'공개된 결성 규모'}],already_covered:[],changes:[],uncertainties:[],angles:empty?[]:[{headline:'추천 제목 '+revision,reason:'공고 조건을 대조한 발제 요지',new_information:'기존 보도에 더한 비교',basis_ids:['f1'],question:'같은 펀드인가',missing:'최종 배분은 미공개',first_action:'원문 확인',direction_key:'fund-terms'}]}});
