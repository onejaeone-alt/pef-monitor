...
test('a concrete fact already covered by MarketIN is not recycled into the daily shelf',()=>{
 const r=result({}, {already_covered:[{text:'홈플러스는 67개점 통매각과 폐점 19개점 별도매각에 착수',source_id:'m1'}]});assert.equal(G.hasNovelSignal(r.analysis.angles[0],r.analysis),false);assert.equal(G.evaluate({headline:'홈플러스'},r,now),null);
});
...
test('single-source, MarketIN-only hard fact or no MarketIN comparison never reaches the daily shelf',()=>{
 ...
 const own=result();own.sources=own.sources.map(s=>s.source_id==='s1'?{...s,url:'https://marketin.edaily.co.kr/News/Read?x=2',is_marketin:true}:s);assert.equal(G.evaluate({headline:'홈플러스'},own,now),null);
});
